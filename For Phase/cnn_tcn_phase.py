...
class resnet_lstm(torch.nn.Module):
    def __init__(self):
        ...
        self.tcn = SingleStageModel()
        #self.lstm2 = nn.LSTM(16, 128, batch_first=True)
        # 128种特征值的样本输入转变成7种特征值的输出
        self.fc = nn.Linear(128, 3)  ####注意修改类别
        #self.tcn = MultiStageModel() #@@@


        '''init.xavier_normal_(self.lstm.all_weights[0][0])  # 正态分布
        init.xavier_normal_(self.lstm.all_weights[0][1])
#        init.xavier_normal_(self.lstm2.all_weights[0][0])
  #      init.xavier_normal_(self.lstm2.all_weights[0][1])
        init.xavier_uniform_(self.fc.weight)  # 均匀分布'''
    ...
    def forward(self, x):#, d
        ...
        x = x.view(-1, 512) # -1表示一个不确定的数,不确定想要reshape成几行
        # print(x.shape) # [128, 512]
        x = x.view(-1, 512, sequence_length)
        #print(x.shape)  # [128, 512, 1], [48, 512, 1]
        #x = x.view(-1, sequence_length, 512)  # 使用batch_first，我们可以将输入变成(batch，seq length，input size)
        # print(x.shape) # [8, 16, 512]   8*16=128
        #self.lstm.flatten_parameters()  # 提高内存的利用率和效率，调用flatten_parameters让parameter的数据存放成contiguous chunk(连续的块)
        #y, _ = self.lstm(x)
        # print(y.shape) # [8, 16, 128]
        ...
        #y = self.fc(y)
        # print(y.shape) # [128,7]
        #yy = y.contiguous().view(-1, sequence_length, 7)
        #print(yy.shape) [8,16,7]  8*16=128
        # y = y.view(y, 1)
        z = self.tcn(x)
        #print('11111',z.shape) #[128, 7, 1]
        # y = y.view(-1, 64, 128)
        # y = self.tcn(y)
        #z = z.contiguous().view(128, -1) #[128, 7]
       #print('222222', z.shape)
        return z
...
class MultiStageModel(nn.Module):
    def __init__(self):
        self.num_stages = 2  # 4 #2
        self.num_layers = 5  # 10  #5
        self.num_f_maps = 64  # 64 #64
        self.dim = 512  #2048 # 2048
        self.num_classes = 7  # 7
        self.causal_conv = 'store_true'
        print(
            f"num_stages_classification: {self.num_stages}, num_layers: {self.num_layers}, num_f_maps:"
            f" {self.num_f_maps}, dim: {self.dim}")
        super(MultiStageModel, self).__init__()
        #self.stage1 =resnet_lstm()
    ...
    def forward(self, x):
        ...
        print('222222', out_classes.shape)
...
class SingleStageModel(nn.Module):
    def __init__(self):
        ...
        self.conv_out_classes = nn.Conv1d(self.num_f_maps, self.num_classes, 1)
        #self.fc = nn.Linear(self.num_f_maps, self.num_classes)

    def forward(self, x):
        out = self.conv_1x1(x)  # [128, 128, 1]
        #print('11111111',out.shape)
        for layer in self.layers:
            out = layer(out)
        out_classes = self.conv_out_classes(out)  # [128, 7, 1]

        #out_classes = out_classes.squeeze(2)
        #print('22222222', out_classes.shape)
        out_classes = out_classes.contiguous().view(-1, 3)
...
def get_data():
    ...
    train_num_each = frames[0: math.floor(video_num * train_ratio)]  # 0:10, train_ratio = 0.6
    # print(len(train_num_each)) 10
    val_num_each = frames[math.floor(video_num * train_ratio): math.floor(video_num * (train_ratio + val_ratio))]  # 10:13
