...
parser.add_argument('--annotation', default='F:/MISAW_test/Procedural decription/annotation.txt', type=str, help='annotation path')
...
def get_useful_start_idx(sequence_length, test_num):
    idx = []
    '''for j in range(0, (test_num + 1 - sequence_length * srate), sequence_length * srate):  # start ID在最后几个构不成一个batch
        idx.append(j)
    return idx'''
    for j in range(0, (test_num - (sequence_length-1)*srate)):
        idx.append(j)
    return idx
...
def test_model(test_dataset):
    ...
    test_useful_start_idx = get_useful_start_idx(sequence_length, num_test)
    #print('1111111', test_useful_start_idx)
    test_idx = []
    for i in test_useful_start_idx:
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            for i in range(len(outputs_s)):
                all_preds_s.append(preds_s[i].data.item())
                
    return all_preds_s


def main():
    step_dict = {0:'Idle', 1:'Needle holding', 2:'Suture making', 3:'Suture handling', 4:'1 knot', 5:'2 knot', 6:'3 knot'}

    test_datasets, num_each = get_data()
    for i in range(len(num_each)):
        s = test_model(test_datasets[i])
        fname = str(i)
        f = output_path +'/'+ fname + '_Results_Step.txt' ################记得改掉split
        c_phase = 0
        c_step = 0
        flag = 0
        change = False
        for i in range(len(s)):
            if s[i] <= c_step + 1 and c_step < 3:
                c_step = s[i]
            s[i] = c_step
        ...
        with open(f, 'w') as of:
            of.write(
                'Frame' + '\t' + 'Step' + '\n')
            fnum = fnum + 1
            for i in range(srate * (sequence_length - 1)):
                of.write(str(
                    fnum) + '\t' + 'Idle' + '\t' + 'Idle' + '\t' + 'Idle' + '\t' + 'Idle' + '\t' + 'Idle' + '\t' + 'Idle' + '\t' + 'Idle' + '\t' + 'Idle' + '\n')
                fnum = fnum + 1
            for i in range(len(s)):
                ous = step_dict[s[i]]
