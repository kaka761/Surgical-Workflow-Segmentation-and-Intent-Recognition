...
class res34_tcn(torch.nn.Module):
    def __init__(self):
        ...
        self.tcn1 = EDTCN(512, [128, 160])
        self.tcn2 = EDTCN(16, [128, 160])
        self.conv_out_classes = nn.Conv1d(128, 7, 1)
        self.tcn3 = MultiTCN(512, 7, [128, 160])
...
class MultiTCN(nn.Module):
    def __init__(self, in_channels, n_classes, mid_channels):
        super(MultiTCN, self).__init__()
        self.n_stages = 4
        self.in_channels = in_channels
        self.n_classes = n_classes
        self.mid_channels = mid_channels
        self.stages = nn.ModuleList(EDTCNs(self.n_classes, self.n_classes, self.mid_channels) for s in range(self.n_stages - 1))
        self.smoothing = False

    def forward(self, x):
        out = x # 1.[8, 7, 16]
        outputs = out.unsqueeze(0)  # 2.[1, 8, 7, 16]
        for s in self.stages:
            out = s(F.softmax(out, dim=1))  # 3.[8, 7, 16];     6.[32, 16, 1]; 9.[32, 16, 1];
            outputs = torch.cat((outputs, out.unsqueeze(0)), dim=0)  # 4.[2, 8, 7, 16]; 7.[3, 32, 16, 1]; 10.[3, 32, 16, 1];
            z= outputs.contiguous().view(self.n_stages, -1, self.n_classes) # [4, 64, 7] # [4, 96, 7] # [4, 128, 7]
        return z
...
class EDTCN(nn.Module):
    def __init__(self, in_channels, mid_channels, kernel_size=25):
        super().__init__()
...
class EDTCNs(nn.Module):
    def __init__(self, in_channels, n_classes, mid_channels, kernel_size=25):
        super().__init__()
