...
class res34_tcn(torch.nn.Module):
    def __init__(self):
        ...
        self.tcn3 = MultiTCN(3, 5, 64, 512, 7)
...
def train_model(train_dataset, train_num_each, val_dataset, val_num_each):
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            ...
            #loss = 0.1 * loss1 + 0.2 * loss2 + 0.7 * loss3
            loss = 0.05 * loss1 + 0.15 * loss2 + 0.8 * loss3
            loss.backward()
            optimizer.step()
            train_loss += loss.data
            train_corrects1 += torch.sum(preds[-3] == labels.data)
            train_corrects2 += torch.sum(preds[-2] == labels.data)
        ...
        for data in val_loader:
            ...
            val_corrects1 += torch.sum(preds[-3] == labels.data)
            val_corrects2 += torch.sum(preds[-2] == labels.data)
...
#########################
        ...
        record_np[epoch, 4] = val_accuracy
        record_np[epoch, 5] = val_average_loss
