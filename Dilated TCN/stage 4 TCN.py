...
class res34_tcn(torch.nn.Module):
    def __init__(self):
        ...
        self.tcn1 = TCN(5, 64, 512)
        self.tcn2 = TCN(5, 64, 16)
        self.conv_out_classes = nn.Conv1d(64, 7, 1)
        self.tcn3 = MultiTCN(4, 5, 64, 512, 7)
...
class MultiTCN(nn.Module):
    def __init__(self, num_stages, num_layers, num_f_maps, dim, num_classes, causal_conv=True):
        ...
        print(f"num_stages_classification: {self.num_stages}, num_layers: {self.num_layers}, num_f_maps:"
                   f" {self.num_f_maps}, dim: {self.dim}")
        super(MultiTCN, self).__init__()
        #self.stage1 = TCN(self.num_layers, self.num_f_maps, self.dim, self.num_classes, causal_conv=self.causal_conv)
        self.stages = nn.ModuleList(TCNs(self.num_layers, self.num_f_maps,  self.num_classes, self.num_classes,
                                 causal_conv=self.causal_conv) for s in range(self.num_stages - 1))
        self.smoothing = False

    def forward(self, x):
        out_classes = x # 1.[32, 16, 1]
        #print('111111',out_classes.shape)
        outputs_classes = out_classes.unsqueeze(0)  # 2.[1, 32, 16, 1]
        for s in self.stages:
            out_classes = s(F.softmax(out_classes, dim=1))  # 3.[32, 16, 1]; 6.[32, 16, 1]; 9.[32, 16, 1];
            outputs_classes = torch.cat((outputs_classes, out_classes.unsqueeze(0)), dim=0)  # 4.[2, 32, 16, 1]; 7.[3, 32, 16, 1]; 10.[3, 32, 16, 1];
            out= outputs_classes.contiguous().view(self.num_stages, -1, self.num_classes)
        return out



class TCN(nn.Module):
    def __init__(self, num_layers, num_f_maps, dim, causal_conv=False):
        super(TCN, self).__init__()
        self.conv_1x1 = nn.Conv1d(dim, num_f_maps, 1)

        self.layers = nn.ModuleList([copy.deepcopy(DilatedResidualLayer(2**i, num_f_maps, num_f_maps, causal_conv))
    ...
    def forward(self, x):
        ...
        #print('11111', z.shape)
        return z
        #out_classes = self.conv_out_classes(out)  # [32, 16, 1]
        #return out_classes
...
def train_model(train_dataset, train_num_each, val_dataset, val_num_each):
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            ...
            '''#all_label = torch.add(labels , preds[-1])
            all_label = torch.cat([labels , preds[-1]], dim=-1)
            print(all_label)
            unique_label = list(set(all_label))
            unique_label.sort()'''
            ...
            #loss = 0.1 * loss1 + 0.2 * loss2 + 0.3 * loss3 + 0.4 * loss4
            # loss = 0.05 * loss1 + 0.15 * loss2 + 0.35 * loss3 + 0.45 * loss4
            loss = 0.05 * loss1 + 0.1 * loss2 + 0.15 * loss3 + 0.7 * loss4
            #loss = 0.05 * loss1 + 0.1 * loss2 + 0.25 * loss3 + 0.6 * loss4
            loss.backward()
            optimizer.step()
            train_loss += loss.data
            train_corrects1 += torch.sum(preds[-4] == labels.data)
            train_corrects2 += torch.sum(preds[-3] == labels.data)
            train_corrects3 += torch.sum(preds[-2] == labels.data)
        ...
        train_accuracy2 = train_corrects2.cpu().numpy() / train_num
        train_accuracy3 = train_corrects3.cpu().numpy() / train_num
        ...
        for data in val_loader:
            ...
            val_corrects1 += torch.sum(preds[-4] == labels.data)
            val_corrects2 += torch.sum(preds[-3] == labels.data)
            val_corrects3 += torch.sum(preds[-2] == labels.data)
...
#########################
        ...
        val_accuracy2 = val_corrects2.cpu().numpy() / val_num
        val_accuracy3 = val_corrects3.cpu().numpy() / val_num
        ...
        record_np[epoch, 3] = val_accuracy2
        record_np[epoch, 4] = val_accuracy3
        record_np[epoch, 5] = val_accuracy

        record_np[epoch, 6] = val_average_loss
